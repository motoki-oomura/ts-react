...
import {fetchCallback} from "@sentry/tracing/types/browser/request";
...
const usePagination = (options: UsePaginationOptions) => {
    ...
    const isViewIndex = useCallback((index: number) => {
        return startCount <= (index + 1) && (index + 1) <= endCount;
    }, [startCount, endCount]);

    const onNextPaginate = useCallback(async (fetchCallback?: ((newPage: number) => Promise<unknown>)) => {
    ...
    const onPrevPaginate = useCallback(async (fetchCallback?: ((newPage: number) => Promise<unknown>)) => {
    ...
    const renderPaginate = ({ next, prev, normal, active }: RenderPaginateProps): React.ReactNode => {
        ...
        return (
            <>
                ...
                { pageList.map(p => <React.Fragment key={`pagination_${p}`}>{p === page ? active(p) : normal(p)}</React.Fragment>)}
