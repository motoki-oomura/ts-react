...
const useApi = <T = unknown>({
    ...
    const callApi = useCallback(
        async (fn: HttpClientMethod, path: Input, options: Options = {}) => {
            setLoading(true);
            let data = null;
    ...
    const api = useMemo(() => {
        return {
            get: (path: Input, options?: Options) =>
                callApi(client.get, path, options),
            post: (path: Input, json: unknown, options?: Options) =>
                callApi(client.post, path, { json, ...options }),
            put: (path: Input, json: unknown, options?: Options) =>
                callApi(client.put, path, { json, ...options }),
            delete: (path: Input, json: unknown, options?: Options) =>
                callApi(client.delete, path, { json, ...options }),
            patch: (path: Input, json: unknown, options?: Options) =>
                callApi(client.patch, path, { json, ...options }),
